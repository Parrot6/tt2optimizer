...
var upgradeButtonChoice = {One: .01, Five: .05, Ten: .10, All: 1};
...
function updateUpgradeButton(value){
  switch(value){
    ...
    case "5":
      ...
      break;
    case "10":
      selectedUpgradeChoice = upgradeButtonChoice.Ten;
      break;
    default:
      selectedUpgradeChoice = upgradeButtonChoice.Five;
      break;
...
var artifactTypeConversions = {
  ...
  TitanDamage: Types.all,
  TitanSlayer: Types.titanslayer,
  UnskilledGold: Types.Gold.Rogue,
...
async function artifactImport(pastedIn){
  ...
  $.ajax({
              type: "GET",
              url: "https://mb1zattts4.execute-api.us-east-1.amazonaws.com/dev/tt2optimizer/assets/ArtifactInfo.csv",
              dataType: "text",
...
function processArtifacts(data){
    ...
    for (var i = 1; i < allTextLines.length; i++ ) {
        ...
        for (var j=0; j<headings.length; j++) {
            if(currentLine[j]) tarr[headings[j]] = currentLine[j].trim();
        }
       // console.log(tarr['Name'])
        var namestr = tarr['Name'] !== "Lucky Foot of Al-Mi-Raj" ? tarr['Name'] : "Lucky Foot of Al-mi'raj";
        //console.log(sampleArtifacts[namestr])
    ...
    var testttt = {};
    Object.keys(ArtifactJsonArray).forEach(function(k){
        testttt[ArtifactJsonArray[k]['BonusType']] = ArtifactJsonArray[k]['BonusType'];
    });
  ...
  Object.keys(ArtifactJsonArray).forEach(function(k){
    //  console.log(ArtifactJsonArray);
    //console.log(k + ' - ' + collection.sampleArtifacts[k]);
...
function availableRelicsUpdated(inputBox) {
  ...
  markArtifactStart();
  console.log(relicsAvailableString);
}
function calculateArtifactPreEfficiencies(artiRow){
    ...
    //console.log(artiRow.Name, artiRow.Effect);
...
function calculateArtifactEfficiencies(artiRow){
    ...
    if(reductionAmt == 0) {
      //console.log(artiRow['BonusType'], reductionAmt)
...
var toUpgradeSummed = [];
var toUpgradeSummedArray = [];
function startUpgradeSuggestions(){
  var upgraded = false;
  Object.entries(ArtifactJsonArray).forEach(([key, value]) => {
    if(value.Rank == 1 && Number(relicsAvailableNumber)>(.05*Number(relicsAvailableString))){
       upgradeNext(value);
       upgraded = true;
    }
  })
  if(upgraded) {
    startUpgradeSuggestions();
      ...
      for(var i = 0; i < toUpgrade.length; i++){
        if(toUpgradeSummed.hasOwnProperty(toUpgrade[i].skill)){
          toUpgradeSummed[toUpgrade[i].skill].cost = printBigScientific(addScientific(toUpgrade[i].cost, toUpgradeSummed[toUpgrade[i].skill].cost));
          toUpgradeSummed[toUpgrade[i].skill].newlevel = Math.max(Number(toUpgrade[i].newlevel), Number(toUpgradeSummed[toUpgrade[i].skill].newlevel));
        } else {
          toUpgradeSummed[toUpgrade[i].skill] = JSON.parse(JSON.stringify(toUpgrade[i]));
        }
        toUpgradeSummed[toUpgrade[i].skill].totalPerc = ( (Number(toUpgradeSummed[toUpgrade[i].skill].cost)/Number(relicsAvailableString))*100).toFixed(2)+"%";
      }
    //   Object.entries(toUpgradeSummed).forEach(([key, value]) => {
    //     toUpgradeSummedArray.push(value);
    //   })
    //   toUpgradeSummedArray.sort(function (a, b) {
    ...
    // toUpgradeSummedArray.forEach(obv => {
    //   var newArt = new model.ArtifactToLevel(obv);
    //   //console.log(newArt);
    //   artis.add(
    //       newArt
    //   )
...
function markArtifactStart(){
  ...
  Object.entries(ArtifactJsonArray).forEach(([key, value]) => {
    ArtifactJsonArray[key].ImportedLevel = Number(ArtifactJsonArray[key].Level);
  })
  startUpgradeSuggestions();
}
...
views.Artifact = Backbone.View.extend({
  ...
  render: function() {
    ...
    //jQuery(this.el).append(jQuery('<button class="col artifactButton 5perc">' + '5%' + '</button>'));
    // var enchantedString = String(this.model.get('Enchanted')).toLowerCase() == "true" ? "Enchanted" : "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";
    // jQuery(this.el).append(jQuery('<div class="col artifactEnchanted text-center">' + enchantedString + '</div>'));
...
views.Artifacts = Backbone.View.extend({
  ...
  render: function() {
    ...
    var artis = artifactsOwned >= 98 ? 98 : artifactsOwned;
    var enchants = artifactsOwned >= 98 ? (artifactsOwned%98) : 0;
    document.getElementById('artifactsDiscovered').innerText = "Artifacts: " + artis + "/98";
    document.getElementById('enchantmentsDiscovered').innerText = "Enchantments: " + enchants + "/34";
    return this;
  }
});
